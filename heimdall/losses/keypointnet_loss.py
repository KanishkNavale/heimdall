...
class KeypointNetLosses:
    ...
    @staticmethod
    def _compute_spatial_expectations(
        depth: torch.Tensor, mask: torch.Tensor, spatial_probs: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        us = torch.arange(
            0, depth.shape[-2], 1, dtype=torch.float32, device=depth.device
        )
        vs = torch.arange(
            0, depth.shape[-1], 1, dtype=torch.float32, device=depth.device
        )
        ...
        exp_u = torch.sum(
            spatial_probs * grid[0].unsqueeze(dim=0).tile(number_of_keypoints, 1, 1),
            dim=(-2, -1),
        )
        exp_v = torch.sum(
            spatial_probs * grid[1].unsqueeze(dim=0).tile(number_of_keypoints, 1, 1),
            dim=(-2, -1),
        )
    ...
    @staticmethod
    def _batch_convert_image_to_camera_coordinates(
        u: torch.Tensor, v: torch.Tensor, d: torch.Tensor, intrinsic: torch.Tensor
    ) -> torch.Tensor:
        du = u * d
        dv = v * d
    ...
    def _compute_multiview_consistency_loss(
        self,
        ...
        one_padded_cam_coords_a = torch.hstack(
            [
                cam_coords_a.squeeze(dim=-1),
                torch.ones_like(cam_coords_a.squeeze(dim=-1)[:, :1]),
            ]
        ).unsqueeze(dim=-1)
        one_padded_cam_coords_b = torch.hstack(
            [
                cam_coords_b.squeeze(dim=-1),
                torch.ones_like(cam_coords_b.squeeze(dim=-1)[:, :1]),
            ]
        ).unsqueeze(dim=-1)

        tiled_extrinsics_a = extrinsic_a.unsqueeze(dim=0).tile(
            cam_coords_a.shape[0], 1, 1
        )
        tiled_extrinsics_b = extrinsic_b.unsqueeze(dim=0).tile(
            cam_coords_b.shape[0], 1, 1
        )
    ...
    def _margin_loss(matrix: torch.Tensor, margin: float) -> torch.Tensor:
        ...
        upper_triangle_indices_x, upper_triangle_indices_y = torch.triu_indices(
            intra_distance.shape[0], intra_distance.shape[1], offset=1
        )
        upper_triangle_matrix = intra_distance[
            upper_triangle_indices_x, upper_triangle_indices_y
        ]
    ...
    def _compute_separation_loss(
        ...
        return 0.5 * (
            self._margin_loss(points_a, self.config.margin)
            + self._margin_loss(points_b, self.config.margin)
        )

    def _compute_silhoutte_loss(
        self, exp_mask_a: torch.Tensor, exp_mask_b: torch.Tensor
    ) -> torch.Tensor:
        return 0.5 * (
            torch.mean(-torch.log(exp_mask_a + 1e-12))
            + torch.mean(-torch.log(exp_mask_b + 1e-12))
        )

    @staticmethod
    def _variance_expectation(
        exp_pixel: torch.Tensor, spatial_probs: torch.Tensor
    ) -> torch.Tensor:
        us = torch.arange(
            0,
            spatial_probs.shape[-2],
            1,
            dtype=torch.float32,
            device=spatial_probs.device,
        )
        vs = torch.arange(
            0,
            spatial_probs.shape[-1],
            1,
            dtype=torch.float32,
            device=spatial_probs.device,
        )
        ...
        tiled_spatial_grid = spatial_grid.unsqueeze(dim=0).tile(
            exp_pixel.shape[0], 1, 1, 1
        )
        tiled_exp_pixel = exp_pixel.view(exp_pixel.shape[0], 1, 1, exp_pixel.shape[1])

        distances_summed = torch.linalg.norm(
            tiled_spatial_grid - tiled_exp_pixel, dim=-1
        )
    ...
    def _compute_variance_loss(
        ...
        spatial_exp_b: torch.Tensor,
    ) -> torch.Tensor:
        return 0.5 * (
            self._variance_expectation(exp_uv_a, spatial_exp_a)
            + self._variance_expectation(exp_uv_b, spatial_exp_b)
        )

    @staticmethod
    def _geodesic_distance(
        rotation_matrix_a: torch.Tensor, rotation_matrix_b: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _compute_relative_pose_loss(
        self,
        cam_coords_a: torch.Tensor,
        extrinsic_a: torch.Tensor,
        cam_coords_b: torch.Tensor,
        ...
        cam_coords_a += torch.normal(
            0.0,
            noise_std,
            size=cam_coords_a.size(),
            device=cam_coords_a.device,
            dtype=cam_coords_a.dtype,
        )
        cam_coords_b += torch.normal(
            0.0,
            noise_std,
            size=cam_coords_b.size(),
            device=cam_coords_b.device,
            dtype=cam_coords_b.dtype,
        )
        ...
        identity_matrix_a = identity_matrix_b = torch.eye(
            3, dtype=cam_coords_a.dtype, device=cam_coords_a.device
        )
        ...
        # Compute loss
        rotation_loss = 0.5 * (
            self._geodesic_distance(rot_trafo_a_to_b, predicted_rot_a_to_b)
            + self._geodesic_distance(rot_trafo_b_to_a, predicted_rot_b_to_a)
        )
    ...
    def _reduce_list(self, list_of_tensors: List[torch.Tensor]) -> torch.Tensor:
        return (
            torch.sum(torch.vstack(list_of_tensors))
            if self.config.reduction == "sum"
            else torch.mean(torch.vstack(list_of_tensors))
        )
    ...
    def _compute_weighted_losses(
        ...
        sihoutte_loss: List[torch.Tensor],
    ) -> torch.Tensor:
        loss = torch.hstack(
            [
                self._reduce_list(consistency_loss),
    ...
    def __call__(
        ...
        for (
            ...
            exp_ua, exp_va, exp_da, exp_ma = self._compute_spatial_expectations(
                depth_a, mask_a, spatial_prob_a
            )
            exp_ub, exp_vb, exp_db, exp_mb = self._compute_spatial_expectations(
                depth_b, mask_b, spatial_prob_b
            )

            cam_coords_a = self._batch_convert_image_to_camera_coordinates(
                exp_ua, exp_va, exp_da, intrinsic_a
            )
            cam_coords_b = self._batch_convert_image_to_camera_coordinates(
                exp_ub, exp_vb, exp_db, intrinsic_b
            )

            consistency_loss.append(
                self._compute_multiview_consistency_loss(
                    cam_coords_a, extrinsic_a, cam_coords_b, extrinsic_b
                )
            )

            separation_loss.append(
                self._compute_separation_loss(
                    torch.vstack([exp_ua, exp_va]).permute(1, 0),
            ...
            pose_loss.append(
                self._compute_relative_pose_loss(
                    cam_coords_a.squeeze(dim=-1),
                    extrinsic_a,
                    cam_coords_b.squeeze(dim=-1),
                    extrinsic_b,
        ...
        weighted_batch_loss = self._compute_weighted_losses(
            consistency_loss, pose_loss, separation_loss, silhoutte_loss
        )

        return {
            "Total": torch.sum(weighted_batch_loss),
